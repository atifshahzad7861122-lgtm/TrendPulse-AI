...
class XtractoShopifyProvider(ShopifyProvider):
    name = "xtracto"
    priority = 4
    display_name = "Xtracto Shopify Product Scraper"
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=self.timeout) as resp:
                if resp.status == 200:
                    ...
                    return ShopifyFetchResult(
                        success=False,
                        error_code=429,
                        error_message="Rate limit reached on Xtracto Scraper",
                    ...
                    return ShopifyFetchResult(
                        ...
                        error_message=f"HTTP {resp.status} on Xtracto Scraper"
                    )
            ...
            if e.code == 429:
                return ShopifyFetchResult(
                    success=False,
                    error_code=429,
                    error_message="Rate limit exceeded on Xtracto Scraper",
                    is_rate_limited=True
                )
            return ShopifyFetchResult(
                success=False,
                error_code=e.code,
                error_message=f"HTTP Error {e.code} on Xtracto: {e.reason}"
            )
            ...
            return ShopifyFetchResult(
                ...
                error_message=f"Network error on Xtracto: {str(e.reason)}",
            ...
            return ShopifyFetchResult(
                success=False,
                error_code=500,
                error_message=f"Unexpected error in Xtracto: {str(e)}"
            )
